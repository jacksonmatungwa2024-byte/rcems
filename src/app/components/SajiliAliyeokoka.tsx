...
const SajiliAliyeokoka: React.FC<SajiliAliyeokokaProps> = ({ setActiveTab }) => {
  ...
  const handleSajiliWokovu = async () => {
    ...
    if (existing && existing.length > 0) {
      setMessage(
        "⚠️ Muumini huyu ameshasajiliwa wokovu. Kuokoka ni mara moja, basi uishi wokovu 🙏"
      );
      setLoading(false);
      return;
    }
    ...
    if (error) {
      ...
      setTimeout(() => setActiveTab("home"), 2000); // ✅ safe with union type
    }
    ...
    <div className="wrapper">
      ...
      {muumini && (
        <div className="result">
          <p>
            <strong>Namba:</strong> {muumini.muumini_namba}
          </p>
          <p>
            <strong>Majina:</strong> {muumini.majina}
          </p>
          <button
            className="button"
